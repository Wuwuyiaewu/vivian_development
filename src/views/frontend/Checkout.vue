<template>
  <div>
    <div class="container pc d-none d-sm-none d-md-block position-relative">
      ...
      <div
        class="row page-transition position-absolute"
        :class="{'hild-left':!pageone}"
      >
        <div
          class="col-7"
          v-if="cartbag.carts.length === 0 "
        >
          購物車內還沒有商品唷
        </div>
        <div
        ...
        <div class="col-5">
          <table class="table">
            ...
            <tbody>
              <td v-if="cartbag.final_total == cartbag.total">
                {{ cartbag.total | currency }} 元
              </td>
              <td
                v-if="cartbag.final_total !== cartbag.total"
                class="text-danger"
              >
                <del>{{ cartbag.total | currency }}</del>
                <div v-if="cartbag.carts.coupon">
                  優惠折扣
                </div>
              </td>
              <td v-if="cartbag.final_total !== cartbag.total">
                {{ cartbag.final_total | currency }} 元
              </td>
            </tbody>
            <tfoot>
              <td colspan="4">
            ...
            <thead>
              <td v-if="cartbag.carts.length !== 0">
                <button
                  ...
                  @click="pageone = !pageone,pagetwo = !pagetwo"
                >
                  下一步
                </button>
              </td>
